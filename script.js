...
// ── SECTION REVEAL (IAX, AFILIADOS) ───────────────────────────
(function () {
  const sections = document.querySelectorAll('.iax-content, .iax-visual, .afiliados-inner, .section-header');
...
(function () {
  ...
  // Just add a blinking cursor class after load for flavor
  window.addEventListener('load', () => {
    setTimeout(() => sub.style.borderRight = 'none', 3000);
  });
})();
...
console.log('%c SYNTRA DIGITAL ', 'background: linear-gradient(135deg, #2979FF, #7C3AED); color: #fff; font-size: 14px; font-weight: bold; padding: 6px 12px; border-radius: 6px;');
console.log('%c syntra.tec.br — Ecossistema Digital Premium ', 'color: #D4A017; font-size: 11px;');
